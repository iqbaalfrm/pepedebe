<?php 
	...
	if (isset($_POST['tambahDataSiswa'])) {
		...
		$check_nisn_query = "SELECT NISN FROM identitas_siswa WHERE NISN = '$nisn'";
		$result = mysqli_query($conn, $check_nisn_query);
	
		if (mysqli_num_rows($result) > 0) {
			echo "Error: NISN already exists. Please use a different NISN.";
		} else {
			// SQL query to insert data into the database
			$query = "INSERT INTO identitas_siswa (NISN, No_KK, NIK, Nama_Panggilan, Nama_Peserta_Didik, Tempat_Lahir, Tanggal_Lahir, Jenis_Kelamin, Agama, Jurusan, Alamat_Tinggal, Provinsi_Tinggal, Kab_Kota_Tinggal, Kec_Tinggal, Kelurahan_Tinggal, Kode_POS, Jarak_ke_Sekolah, Riwayat_Penyakit, tgl_buat) 
					  VALUES ('$nisn', '$no_kk', '$nik', '$nama_panggilan', '$nama_lengkap', '$tempat_lahir', '$tanggal_lahir', '$jenis_kelamin', '$agama', '$jurusan', '$alamat_tinggal', '$provinsi_tinggal', '$kab_kota_tinggal', '$kecamatan_tinggal', '$kelurahan_tinggal', '$kode_pos', '$jarak_ke_sekolah', '$riwayat_penyakit', '$tgl_buat')";
